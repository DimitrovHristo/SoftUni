...
class SequenceInMatrix
{
    static void Main(string[] args)
    {
        ...
        for (int i = 0; i < rows - 1; i++)
        {
            ...
            {
                CheckSequence(CheckRow(matrix, i, j), ref counter, ref bestString, matrix, i, j);
                CheckSequence(CheckCol(matrix, i, j), ref counter, ref bestString, matrix, i, j);
                CheckSequence(CheckDiagonal(matrix, i, j), ref counter, ref bestString, matrix, i, j);
            }
        }

        if (bestString.Count == 1)
        {
            Console.WriteLine("The longest sequence of equal strings in the matrix is:", counter);
